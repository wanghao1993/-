---
name: write-developer-growth-wechat
description: 为「说点AI」创作、改写、审核和复盘面向开发者的简体中文公众号内容，支持选题规划、系列策划、标题筛选、实战教程、源码分析、AI 工具与 Agent 工作流解读、微信发布稿、分发素材和发布后数据复盘。在目标是提高点击、完读、分享、收藏、关注转化和复访，或把账号长期做到 1 万真实关注者时使用。强制要求真实实验、可验证证据、明确边界和系列化增长，不通过水文、夸大标题或照搬他人经历换取流量。
---

# 开发者公众号增长写作

用值得点击的承诺吸引目标读者，用可验证的技术内容促成完读、分享、收藏和关注。长期目标是把「说点AI」做成至少 1 万名真实关注者的开发者账号，但不用失真、照搬、诱导互动或低质高频换取增长。

## 使用配套资料

- 选择内容类型、标题写法或文章结构时，读取 [references/benchmark.md](references/benchmark.md)。
- 规划账号、系列、文章长度、关注转化或发布后复盘时，读取 [references/growth-strategy.md](references/growth-strategy.md)。
- 新建文章时，复制并填写 [assets/article-template.md](assets/article-template.md)。

## 写作流程

### 1. 阅读工作区和原始材料

1. 先阅读仓库说明和目标文章目录。
2. 阅读用户提供的笔记、链接、截图、代码、测试结果和相关文章。
3. 只要涉及最新变化、争议、技术事实或数据，就先查一手资料，再写提纲。
4. 记录资料日期、产品版本和依赖版本。旧资料只能作为线索，不能直接当作当前事实。
5. 检查旧文章和已有系列，判断本文是拉新、建立信任、促进关注，还是承接复访。

### 2. 确定一种主要内容类型

每篇文章只选一种主要类型：

| 类型 | 适用场景 | 必须提供的原创内容 |
|---|---|---|
| 快速解读 | 新版本、规则变化、新工具 | 谁需要关心、具体变了什么、下一步怎么做 |
| 实战教程 | 一个明确的开发问题 | 可复现步骤、代码、限制和验证结果 |
| 源码深挖 | Agent、框架或工具内部机制 | 架构、数据流、关键代码和可迁移的经验 |
| 场景讲解 | 难懂的技术概念 | 真实可信的面试、事故或排错场景 |
| 工具盘点 | 工具链或生态梳理 | 选择标准、取舍和推荐路径 |

找不到明确读者、长期或当下价值、个人判断和独有证据时，先不要写。

### 3. 给选题打分

每项按 0～5 分计算：

- 读者痛点：25%
- 时效性或搜索价值：15%
- 原创增量：25%
- 可验证程度：25%
- 系列潜力：10%

总分达到 3.5 分再进入写作。如果原创增量或可验证程度低于 3 分，先补研究或实验。

### 4. 写一页简报

动笔前先写清楚：

- 目标读者
- 增长任务：拉新、信任、转化或复访
- 这篇只解决的一个问题
- 承诺交付的一个结果
- 为什么现在值得写
- 核心结论
- 相比现有资料多提供了什么
- 证据：代码、截图、日志、耗时、成本、对比或一手资料
- 可以接着写的下一篇
- 所属内容支柱和系列位置
- 自然的互动或关注引导

一篇文章只承担一个主要承诺。

### 5. 生成并筛选标题

至少生成 10 个标题，覆盖下面四种写法：

1. 熟悉的工具 + 意外变化
2. 明确痛点 + 具体结果
3. 数字或限制条件 + 明确收益
4. 问题或面试场景 + 背后原理

从清晰度、读者匹配、具体收益、好奇心和正文支撑五个方面评分，每项 0～2 分，推荐总分最高的标题。

除非正文能证明，否则不要使用「重磅」「炸裂」「史上最强」「彻底」或绝对排名。标题承诺的结果必须在正文兑现。

### 6. 写正文

没有特殊原因时，按下面的顺序写：

1. 用 80～160 个汉字写一个具体场景、麻烦、结果或矛盾。
2. 说明这件事为什么和读者有关，并提前给出核心结论。
3. 先给最小可用答案，再逐步解释原理。
4. 主要判断都要有依据，例如可运行代码、截图、输出、来源、表格或推理过程。
5. 说明边界、成本、风险，以及哪些人不适合采用这个方案。
6. 用简短清单或决策建议收尾。
7. 预告一个具体的后续主题，自然邀请讨论或关注。不能用资料交换诱导转发、点赞或点「在看」。

段落要短，方便手机阅读。多用直接的动词和具体名词，优先写亲自观察到的结果。删掉空泛铺垫、夸张判断、重复转折和励志口号。

长度服务于阅读任务，不服务于凑字数。有足够材料的实战教程、工作流复盘和源码分析，发布稿正文通常写到 3500～5500 个汉字，对应约 8～12 分钟阅读。快讯或单点解答可以更短，源码深挖可以更长。需要加长时，只增加证据、实例、推理、对比、失败过程、边界和决策建议。

### 7. 核验技术内容

整理发布稿前逐项检查：

- 版本、基准数据、价格、日期、API 和产品能力是否有一手来源。
- 代码是否实际运行过；如果没有，要明确标注为示意代码，尚未验证。
- 官方说法、第三方测试和作者推断是否分开表达。
- 截图和代码能否支撑正文结论。
- 翻译、改编、引用是否注明来源。
- 使用 AI 生成或修改的图片、音频、视频等内容，是否按当前平台规则完成声明和标识。

### 8. 整理交付文件

遵守工作区的文章目录规则，输出：

- `full-article.md`：完整初稿
- `summary.md`：核心观点、文章结构和可复用句子
- `wechat-ready.md`：标题备选、推荐标题、导语、正文、可选结尾和参考资料
- `images/`：文章专用图片，Markdown 中使用相对路径
- 工作区要求确认稿或导出稿时，再提供 HTML

如果任务包含增长分发，再补充两版朋友圈文案和一份 30～60 秒短视频提纲。

### 9. 发布后复盘

只在用户提供后台数据或要求复盘时执行：

1. 把展现、打开、读完深度、分享、收藏、新增关注、取关、流量来源和有信息量的留言写入文章目录的 `notes.md`。
2. 区分已知事实和推断，不猜测缺失指标。
3. 判断主要瓶颈位于展现、点击、完读、分享还是关注转化。
4. 每次只提出 1～3 个可验证的修改，并将读者问题转成后续选题。

## 交付前检查

下面的问题全部回答「是」，才能完成：

- 开头 10 秒内，开发者能否判断这篇文章和自己是否有关？
- 第一屏是否已经出现具体问题或结果？
- 是否至少有一个无法靠泛泛改写得到的证据？
- 是否同时讲清楚了怎么做和为什么？
- 是否写明限制条件和不适用情况？
- 字数是否由内容和证据支撑，而不是重复扩写？
- 读完本文后，读者是否能清楚记住「说点AI」提供的独特价值？
- 是否与一个后续主题或已有系列自然连接？
- 标题和正文是否一致？
- 文中的经历是否确实属于当前作者，而不是借来的故事？
- 引用和改编内容是否注明来源？
